...
RE::BSEventNotifyControl HitEventWatcher::ProcessEvent(const RE::TESHitEvent& evn, [[maybe_unused]] RE::BSTEventSource<RE::TESHitEvent>* src)
{
	...
	if (evn.hitdata.source.object) {
		...
		if (evn.hitdata.impactData.collisionObj) {
			...
			if (shouldLog) {
				logger::info("---------Damage System Activated-----------");
				...
				*/
				...
				if (projType == 0x40000 || projType == 0x80000 || projType == 0x200000) {  //에너지
					...
					if (partFound <= 2) {
						if (partFound == 1) {  //머리
							...
							if (shouldLog) {
								...
								logger::info("Head Death Condition: {}, Current Head Condition: {}, Condition Damage: {}", deathCondition, victim->GetActorValue(*Globals::perceptionCondition), evn.hitdata.targetedLimbDamagePercent);
							}
							if (deathCondition >= victim->GetActorValue(*Globals::perceptionCondition) - evn.hitdata.targetedLimbDamagePercent || deathCondition >= victim->GetActorValue(*Globals::brainCondition) - evn.hitdata.targetedLimbDamagePercent) {
								chance = victim->GetActorValue(*Globals::perceptionCondition) - evn.hitdata.targetedLimbDamagePercent <= 0 ? 0 : gcd.protectionChances[(int)armorTier];

								if (!victim->HasKeyword(Globals::Head_DeathAvoidKeyword) && !victim->HasKeyword(Globals::DeathAvoidKeyword)) {
									hasDeathChance = true;
								} else {
									if (shouldLog) {
										logger::info("Head Death Protected");
									}
								}
							}
							...
							if (shouldLog) {
								...
								logger::info("Torso Death Condition: {}, Current Torso Condition: {}, Condition Damage: {}", deathCondition, victim->GetActorValue(*Globals::enduranceCondition), evn.hitdata.targetedLimbDamagePercent);
							}
							if (deathCondition >= victim->GetActorValue(*Globals::enduranceCondition) - evn.hitdata.targetedLimbDamagePercent) {
								chance = victim->GetActorValue(*Globals::enduranceCondition) - evn.hitdata.targetedLimbDamagePercent <= 0 ? 0 : gcd.protectionChances[(int)armorTier];

								if (!victim->HasKeyword(Globals::Torso_DeathAvoidKeyword) && !victim->HasKeyword(Globals::DeathAvoidKeyword)) {
									hasDeathChance = true;
								} else {
									if (shouldLog) {
										logger::info("Torso Death Protected");
									}
								}
							}
						}
					...
					if (aeList) {
						...
						if (bld.conditionStart >= victim->GetActorValue(**Globals::EFDConditions[partFound])) {  // 내구도 확인
							...
							if (partFound == 2) {
								std::mt19937 e3{ rd() };
								std::uniform_real_distribution<float> dist3{ 0, 100 };
								float HBLDResult = dist3(e3);
								if (shouldLog) {
									logger::info("HBLD Chance {}, HBLD Avoid {}", bld.HBLDChance, HBLDResult);
								}

								if (HBLDResult && HBLDResult <= bld.HBLDChance) {
									//마법 효과 리스트에서 출혈 효과를 찾아보고 출혈 효과가 있으면 강도 증가 + 갱신, 없으면 새로운 출혈 효과 적용
									RE::ActiveEffect* HBLDae = nullptr;
									for (auto it = aeList->data.begin(); it != aeList->data.end(); ++it) {
										RE::ActiveEffect* ae = it->get();
										if (ae && !(ae->flags & RE::ActiveEffect::Flags::kInactive) && ae->item == bld.HBLDSpell) {
											HBLDae = ae;
											if (shouldLog) {
												logger::info("{} is already Heavy Bleeding", Globals::EFDBodyPartsName[partFound].c_str());
											}
										}
									}
									float bleedmag = evn.hitdata.calculatedBaseDamage * bld.multiplier;
									if (HBLDae) {
										HBLDae->magnitude -= bleedmag;
										HBLDae->elapsed = 0;
										if (shouldLog) {
											logger::info("Current Heavy bleeding magnitude {}", HBLDae->magnitude * -1.0f);
										}
									} else {
										bld.HBLDSpell->listOfEffects[0]->data.magnitude = bld.HBLDInitializeDamage + bleedmag;
										bld.HBLDSpell->listOfEffects[1]->data.magnitude = bld.HBLDInitializeDamage + bleedmag;
										bld.HBLDSpell->Cast(victim, victim, victim, RE::GameVM::GetSingleton()->GetVM().get());
										if (shouldLog) {
											logger::info("Heavy Bleeding start magnitude {}", bld.HBLDInitializeDamage + bleedmag);
										}
									}
								}
							}
					...
					CartridgeData& gcd = Configs::cartridgeData.at("Global");  // 일반 탄약
					if (partFound <= 2) {
						if (partFound == 1) {  //머리
							float armorTier = victim->GetActorValue(*Globals::helmetTier);
							...
							float ActorHeadCondition = 0;
							if (victim->GetActorValue(*Globals::perceptionCondition) <= victim->GetActorValue(*Globals::brainCondition)) {
								ActorHeadCondition = victim->GetActorValue(*Globals::perceptionCondition);
							} else {
								ActorHeadCondition = victim->GetActorValue(*Globals::brainCondition);
							}

							if (deathCondition >= ActorHeadCondition - evn.hitdata.targetedLimbDamagePercent) {
								float protectionChance = (uint16_t)max(min(log10((Configs::helmetRatings[armorTier] * (ActorHeadCondition - evn.hitdata.targetedLimbDamagePercent) / 100) / Configs::gdHead.formulaA + 1.0f) * Configs::gdHead.formulaB + (Configs::gdHead.formulaC - evn.hitdata.calculatedBaseDamage) * 0.25f + Configs::gdHead.formulaD, 100), 0);
								if (victim->GetActorValue(*Globals::perceptionCondition) - evn.hitdata.targetedLimbDamagePercent <= 0 || armorTier == 0 || deathCondition == 100.f) {
									chance = 0;
								} else {
									chance = protectionChance;
								}
							...
							float ActorEnduranceCondition = victim->GetActorValue(*Globals::enduranceCondition);
							if (deathCondition >= ActorEnduranceCondition - evn.hitdata.targetedLimbDamagePercent) {
								float protectionChance = (uint16_t)max(min(log10((Configs::vestRatings[armorTier] * (ActorEnduranceCondition - evn.hitdata.targetedLimbDamagePercent) / 100) / Configs::gdTorso.formulaA + 1.0f) * Configs::gdTorso.formulaB + (Configs::gdTorso.formulaC - evn.hitdata.calculatedBaseDamage) * 0.25f + Configs::gdTorso.formulaD, 100), 0);
								if (victim->GetActorValue(*Globals::enduranceCondition) - evn.hitdata.targetedLimbDamagePercent <= 0 || armorTier == 0 || deathCondition == 100.f) {
									chance = 0;
								} else {
									chance = protectionChance;
								}
				...
				if (hasDeathChance) {
					...
					if (result > chance) {
						...
						if (shouldLog) {
							logger::info("---------Actor is Dead by Instant Kill---------");
						}
					}
					//즉사 회피시 메세지 표기
