...
void SetupBleeding(CSimpleIniA& ini)
{
	...
	//과다출혈
	bldTorso.HBLDSpell = Utils::GetSpellByFullName(std::string("EFD Condition Heavy Bleed Torso"));
	bldTorso.HBLDInitializeDamage = std::stof(ini.GetValue("BleedingTorso", "HBLDInitializeDamage"));
	bldTorso.HBLDChance = std::stof(ini.GetValue("BleedingTorso", "HBLDChance"));
	...
	for (auto it = bldTorso.HBLDSpell->listOfEffects.begin(); it != bldTorso.HBLDSpell->listOfEffects.end(); ++it) {
		(*it)->data.magnitude = bldTorso.HBLDInitializeDamage;
		(*it)->data.duration = (int32_t)floor(bldTorso.duration);
	}
	...
	logger::info("_SpellConditionBleedTorso {:08X}", (uintptr_t)bldTorso.HBLDSpell);
